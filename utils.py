...
def time_str_to_cron(time_str: str) -> str:
    ...
    try:
        ...
        if timezone_offset != 0:
            logger.info(f"检测到时区差异，将调整时间从本地时间 {hour:02d}:{minute:02d} 到cron时间")
        ...
        try:
            ...
            logger.info(f"使用cron表达式 '{cron_expression}' 计算的下次执行时间: {local_next_run.strftime('%Y-%m-%d %H:%M:%S')} (本地时间)")
...
def is_group_enabled(group_id: str, enabled_groups: Set[str]) -> bool:
    ...
    if not enabled_groups:
        logger.debug(f"未指定启用群列表，默认启用所有群聊，包括: {group_id}")
        return True
...
def get_image_path(session_id: str, timestamp: Optional[int] = None) -> Path:
    ...
    if DATA_DIR is None:
        ...
        try:
            ...
            logger.warning(f"DATA_DIR未初始化且无法通过StarTools获取，使用临时目录存储图片: {data_dir}")
    else:
        data_dir = DATA_DIR
...
def segment_text(text: str, min_length: int = 2, stop_words: Optional[List[str]] = None) -> List[str]:
    """
    使用jieba进行中文分词
    ...
    try:
        ...
        if isinstance(session_id, str) and ":" in session_id:
            ...
            if len(parts) >= 3:
                ...
                if ("group" in middle_part or "群" in middle_part or 
                    "multi" in middle_part or "channel" in middle_part):
                    # 提取第三部分作为群号
                    if parts[2].isdigit():
                        logger.debug(f"从三段式会话ID '{session_id}' 提取到群号: {parts[2]}")
                        return parts[2]
            
            # 2. 从会话ID的各部分中寻找可能的群号，优先选择最后一部分
            for i in range(len(parts) - 1, -1, -1):  # 从后向前查找
                if parts[i].isdigit() and len(parts[i]) >= 5:  # 群号通常至少5位
                    logger.debug(f"从会话ID '{session_id}' 的第{i+1}部分提取到可能的群号: {parts[i]}")
                    return parts[i]
